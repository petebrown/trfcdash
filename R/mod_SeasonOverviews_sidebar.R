#' SeasonOverviews_sidebar UI Function
#'
...
mod_SeasonOverviews_sidebar_ui <- function(id){
  ns <- NS(id)
  tagList(
    sliderInput(
    ...
    sliderInput(
      inputId = ns("game_range"),
      label = h6("Select game range:"),
      min = min(results_dataset$game_no),
      max = max(results_dataset$game_no),
      sep = "",
      ticks = FALSE,
      step = 1,
      value = c(
        min(results_dataset$game_no),
        max(results_dataset$game_no)
      )
    ),
...
#' SeasonOverviews_sidebar Server Functions
#'
#' @noRd
mod_SeasonOverviews_sidebar_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    season_overviews_inputs <- list(
      ...
      reactive({input$game_range}) #7
    )
    return(season_overviews_inputs)
