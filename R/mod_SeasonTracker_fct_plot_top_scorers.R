plot_ssn_scorers <- function(selected_season) {
  ...
  ssn_top <- summaries %>%
    ...
    dplyr::mutate(
      ordered = paste0(season, Total, player_name) %>% forcats::fct_inorder()
    ) %>%
    dplyr::arrange(season, Total, desc(player_name))
